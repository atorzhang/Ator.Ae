...
namespace Ator.Site.Controllers
{
    public class QQAuthorController : BaseController
    {
        ...
        //qq登录配置
        public QQAuthor qqAuthor = AppSettings.GetAppSettings<QQAuthor>("QQAuthor");

        public QQAuthorController(UnitOfWork unitOfWork, ILogger<QQAuthorController> logger)
        {
        ...
        public async Task<IActionResult> Return(string code)
        {
            ...
            //判断openid是否在数据库中存在了
            var userModel = _unitOfWork.SysUserRepository.Get(o => o.QQ == openId);
            if(userModel == null)
            {
                //不存在插入
                userModel = new Entity.Sys.SysUser
                {
                    SysUserId = openId,
                ...
                _unitOfWork.SysUserRepository.Insert(userModel);
            }
            else
            {
                //存在更新
                ...
                _unitOfWork.SysUserRepository.Update(userModel);
            }
